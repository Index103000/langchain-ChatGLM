...
class OpenAILLM(RemoteRpcModel, LLM, ABC):
    ...
    def build_message_list(self, query) -> Collection[Dict[str, str]]:
        ...
        logger.info(f"所有的 history 如下：")
        logger.info(self.history)

        logger.info(f"截取的历史数据长度：{self.history_len}")

        history = self.history[-self.history_len:] if self.history_len > 0 else []

        logger.info(f"当前的 history 如下：")
        logger.info(history)
    ...
    def generatorAnswer(self, prompt: str,
        ...
        # 构建消息
        message = self.build_message_list(prompt)
